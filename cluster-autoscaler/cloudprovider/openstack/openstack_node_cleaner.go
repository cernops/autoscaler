...
type NodeCleanerInterface interface {
	CheckNodesAccess() error
	CleanupNodes(nodeNames []string) error
}
...
func (nc *NodeCleaner) CleanupNodes(nodeNames []string) error {
	glog.V(2).Info("Removing nodes from kubernetes node list")
	failedDeletions := []string{}
	for _, nodeName := range nodeNames {
		glog.V(3).Infof("Cleaning up node %s", nodeName)
		err := nc.kubeClient.CoreV1().Nodes().Delete(nodeName, &metav1.DeleteOptions{})
		if err != nil {
...
func (nc *NodeCleaner) CheckNodesAccess() error {
	_, err := nc.kubeClient.CoreV1().Nodes().List(metav1.ListOptions{})
	if err != nil {
